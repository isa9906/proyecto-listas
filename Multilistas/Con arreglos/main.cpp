				...
				case 2: // Eliminar
					if(lista.estaVacia()){
						...
						while(!(posicion>=1 && posicion<=lista.getindice())){
							...
							if(!(posicion>=1 && posicion<=lista.getindice())) cout<<"Dato err贸neo, la lista tiene "<<lista.getindice()<<" elementos. Int茅ntelo de nuevo";
						}
...
void imprimir(Persona *persona,multilista lista,int indice){
	...
	cout<<"No hay elementos con estas caracter韘ticas";
}
}
void imprimirCarrera(Persona *persona,multilista lista, int opcion){
		int indice;
